      ...
      <div class="col-8 offset-2 col-md-8 offset-md-2">
        ...
        <div>
          <div v-for="each in fetchers.novels" :key="each">
            <a :href="'http://' + each" target="_blank">{{ each }}</a>
            <br />
          </div>
        </div>
        <form>
...
export default {
  ...
  methods: {
    ...
    addNovel() {
      ...
      this.$api
        .addNewNovel(this.$vuex.getters.getSession(), this.addNovelUrl)
        .then(data => {
    ...
    addComic() {
      ...
      this.$api
        .addNewComic(this.$vuex.getters.getSession(), this.addComicUrl)
        .then(data => {
          this.$toasted.info("添加成功！");
          console.log(data);
        })
