    ...
    <div id="article" class="container">
      <div v-if="Object.keys(comic).length !== 0" class="row">
        <aside class="col-2 col-md-1">
          <affix
            class="sidebar-menu"
            relative-element-selector="#comic"
            align="left"
          >
            <button
              @click="$router.go(-1)"
...
export default {
  ...
  async created() {
    ...
    if (!this.$vuex.getters.isComicIDExists(this.comicID)) {
      ...
      if (
        localComic.cahpers === undefined ||
        this.$vuex.getters.isComicNeedUpdate(this.comicID)
      ) {
        let comic = await this.$api.fetchComicByID(session, this.comicID);
        this.$vuex.commit("updateComic", {
  ...
  methods: {
    async get() {
      ...
      await this.fetchComicChapter(this.fetchIndex++).then(data =>
    ...
    fetchComicChapter(index) {
      return this.$api
        .fetchComicChapter(this.$vuex.getters.getSession(), this.comicID, index)
        .then(data => {
          return {
            index: index,
            content: data
    ...
    observe(isVisible, entry, section) {
      if (isVisible) {
        if (this.transitIndex < section.index) {
          ...
          this.$vuex.commit("insertComicBookmark", {
            comicID: this.comic.comicID,
            bookmark: this.bookmark
          });
          if (!this.loading && this.fetchIndex - this.currentIndex < 1) {
            this.loading = true;
            this.fetchComicChapter(this.fetchIndex++).then(data =>
              this.sections.push(data)
            );
            setTimeout(() => {
              this.loading = false;
